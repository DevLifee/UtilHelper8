...
@available(iOS 14.0, *)
public struct UtilEight: View {
    // @State private var isShowagenciessay: Bool = false
    public init(listData: [String: String], pushToSeven: @escaping () -> Void, show: Binding<Bool>) {
                    ...
                    VStack(spacing: 10) {
                        HStack {
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                Text(listData[RemoKey.nameappf1.rawValue] ?? "").fontWeight(.bold)
                                Text(listData[RemoKey.titleappf1.rawValue] ?? "")
                                    .font(.system(size: 13))
                        ...
                        HStack(spacing: 5) {
                            ...
                            Text(listData[RemoKey.wereloadingf1.rawValue] ?? "").fontWeight(.semibold)
                            Spacer()
                        }
                        Text(listData[RemoKey.timeref1.rawValue] ?? "").font(.system(size: 15))
                        ...
                        Text(listData[RemoKey.pleasedof1.rawValue] ?? "").foregroundColor(Color.red).padding(.bottom, 20).opacity(0.8)
                    }
                }
                .cornerRadius(10)
